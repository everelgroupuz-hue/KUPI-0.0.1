            ...
            <div key={ret.id} className="bg-white dark:bg-surface-800 rounded-2xl p-4 border border-surface-200 dark:border-surface-700">
              ...
              {ret.status === 'pending' && (
                <div className="flex items-center gap-2 mt-3">
                  ...
                  >
                    {updatingId === ret.id ? <span className="w-4 h-4 border-2 border-success border-t-transparent rounded-full animate-spin" /> : <Check className="w-4 h-4" />}
                  </button>
                  <button
                  ...
                  >
                    {updatingId === ret.id ? <span className="w-4 h-4 border-2 border-danger border-t-transparent rounded-full animate-spin" /> : <X className="w-4 h-4" />}
                  </button>
                </div>
              )}
              {ret.status === 'approved' && (
                <button
                  ...
                  disabled={updatingId === ret.id}
                  className="btn-brand px-4 py-2 rounded-xl text-xs mt-3 disabled:opacity-50"
                >
                  {updatingId === ret.id ? 'Обработка...' : 'Подтвердить возврат'}
                </button>
              )}
            </div>
          ))
